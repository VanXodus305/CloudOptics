...
export default function UtilizationChart({
  ...
  // Filter for EC2 & RDS instances (servers)
  const serverResources = resources.filter((r) => r.service === "EC2" || r.service === "RDS");
  const data = serverResources.slice(0, 6).map((r) => ({
    name: r.resourceId,
    cpu: r.cpu,
    memory: r.memory,
    storage: r.storage,
  ...
  return (
    <div className={`bg-white/90 backdrop-blur-xl rounded-3xl p-6 shadow-xl border border-white/60 w-full flex flex-col relative transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 mt-8 ${drilldownServer ? "h-auto min-h-[500px]" : "h-[380px]"}`}>
      ...
      <div className="flex-grow w-full relative">
        {drilldownServer && serverObj ? (
          <div className="flex flex-col h-full gap-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-2">
              {[
                ...
                {
                  label: "Memory",
                  value: serverObj.memory,
                  color: "text-[#9A4DCC]",
                  bg: "bg-[#9A4DCC]/10",
                },
                {
                  label: "Storage",
                  value: serverObj.storage,
                  color: "text-[#1F215D]",
                  bg: "bg-[#1F215D]/10",
                },
                {
                  label: "Network",
                  value: serverObj.network,
                  color: "text-[#792CA2]",
                  bg: "bg-[#DCCBFF]/40",
                },
              ].map((kpi, idx) => (
            ...
            <div className="flex-grow w-full relative" style={{ height: 260 }}>
              {/* Y-axis label */}
              <div
                className="absolute left-[10px] top-1/2 -translate-y-1/2 -translate-x-1"
                style={{
              ...
              <ResponsiveContainer width="100%" height="100%" className="focus:outline-none">
                <BarChart style={{ outline: "none" }} data={drilldownData} margin={{ top: 20, right: 10, left: 40, bottom: 30 }}>
                  ...
                  />
                  <Bar dataKey="value" radius={[4, 4, 0, 0]} isAnimationActive={true} animationDuration={1000} animationEasing="ease-in-out">
          ...
          <div className="relative w-full h-full flex flex-col">
            ...
            <div className="text-center pb-1" style={{ fontSize: 12, fontWeight: 700, color: "#111844" }}>
              Percentage ➔
            </div>
          </div>
        )}
