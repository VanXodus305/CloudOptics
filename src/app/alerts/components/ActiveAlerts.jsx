...
export default function ActiveAlerts() {
  ...
  const uniqueEnvs = [
    "All",
    ...new Set(alerts.map((a) => a.environment || "Unknown")),
  ];
  ...
  useEffect(() => {
    ...
    const currentFiltered = alerts.filter(
      (alert) => alert.status === tabStatusMap[activeTab],
    );
    ...
    if (currentSorted.length > 0) {
      setSelectedAlert((prev) => {
        const isSelectedInList =
          prev && currentSorted.some((a) => a._id === prev._id);
            ...
            >
              <div className="flex justify-center pb-2">
                <div className="flex items-center gap-2 bg-gray-50/80 dark:bg-white/5 p-1.5 rounded-xl border border-gray-100 dark:border-white/10 shadow-sm w-full xl:w-auto">
                  <button
                    onClick={() =>
                      updateAlertStatus(selectedAlert._id, "unresolved")
                    }
                    className={`flex-1 xl:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg transition-all ${
                      selectedAlert.status === "unresolved"
                        ? "bg-red-100 border border-red-200 shadow-sm"
                        : "hover:bg-red-50 opacity-60 hover:opacity-100 grayscale hover:grayscale-0"
                    }`}
                    title="Mark Unresolved"
                  >
                    <FlagIcon
                      className={`w-4 h-4 ${selectedAlert.status === "unresolved" ? "text-red-500" : "text-gray-400"}`}
                    />
                    <span
                      className={`text-[10px] font-bold ${selectedAlert.status === "unresolved" ? "text-red-700" : "text-gray-500"}`}
                    >
                      Unresolved
                    </span>
                  </button>
                  ...
                  <button
                    onClick={() =>
                      updateAlertStatus(selectedAlert._id, "in progress")
                    }
                    className={`flex-1 xl:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg transition-all ${
                      selectedAlert.status === "in progress"
                        ? "bg-yellow-100 dark:bg-yellow-500/20 border border-yellow-200 dark:border-yellow-500/30 shadow-sm"
                        : "hover:bg-yellow-50 dark:hover:bg-white/5 opacity-60 hover:opacity-100 grayscale hover:grayscale-0"
                    }`}
                    title="Start Progress"
                  >
                    <FlagIcon
                      className={`w-4 h-4 ${selectedAlert.status === "in progress" ? "text-yellow-500" : "text-gray-400"}`}
                    />
                    <span
                      className={`text-[10px] font-bold ${selectedAlert.status === "in progress" ? "text-yellow-700" : "text-gray-500"}`}
                    >
                      In Progress
                    </span>
                  </button>
                  ...
                  <button
                    ...
                    className={`flex-1 xl:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg transition-all ${
                      selectedAlert.status === "resolved"
                        ? "bg-green-100 dark:bg-green-500/20 border border-green-200 dark:border-green-500/30 shadow-sm"
                        : "hover:bg-green-50 dark:hover:bg-white/5 opacity-60 hover:opacity-100 grayscale hover:grayscale-0"
                    }`}
                    title="Mark Resolved"
                  >
                    <FlagIcon
                      className={`w-4 h-4 ${selectedAlert.status === "resolved" ? "text-green-500" : "text-gray-400"}`}
                    />
                    <span
                      className={`text-[10px] font-bold ${selectedAlert.status === "resolved" ? "text-green-700" : "text-gray-500"}`}
                    >
                      Resolved
                    </span>
                  </button>
              ...
              <div>
                <h3 className="text-base font-black text-[#111844] dark:text-white leading-snug">
                ...
                <div className="mt-2.5 flex flex-wrap gap-2 text-[11px] font-semibold text-gray-500 dark:text-gray-400">
                  <span className="bg-gray-100 dark:bg-white/5 px-2 py-0.5 rounded-md border border-gray-200/40 dark:border-white/10 flex items-center gap-1">
                    Resource ID:{" "}
                    <strong className="text-gray-700 dark:text-gray-200 ml-0.5">
                      {selectedAlert.resourceId}
                    </strong>
                    <button
                      onClick={() =>
                        navigator.clipboard.writeText(selectedAlert.resourceId)
                      }
                  ...
                  <span className="bg-gray-100 dark:bg-white/5 px-2 py-0.5 rounded-md border border-gray-200/40 dark:border-white/10 flex items-center">
                    Type:{" "}
                    <strong className="text-gray-700 dark:text-gray-200 ml-1">
                      {selectedAlert.type}
                    </strong>
                  </span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-2">
                <div className="bg-white dark:bg-transparent border border-gray-150 dark:border-white/10 p-3 rounded-xl shadow-sm">
                  ...
                  <p className="text-base font-extrabold text-gray-600 dark:text-gray-300 mt-0.5">
                    ${Math.round(selectedAlert.currentCost || 0)}/mo
                  </p>
                </div>
                <div className="bg-emerald-500/5 border border-emerald-500/15 dark:border-emerald-500/30 p-3 rounded-xl shadow-sm">
                  <p className="text-[9px] font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-wide">
                    Potential Savings
                  </p>
                  <p className="text-base font-black text-emerald-600 dark:text-emerald-500 mt-0.5">
                    ${Math.round(selectedAlert.potentialSavings || 0)}/mo
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-transparent border border-gray-150 dark:border-white/10 rounded-xl p-4 shadow-sm flex-grow">
                <h4 className="text-[10px] font-black text-[#111844] dark:text-gray-200 uppercase tracking-wider mb-2">
                  Recommended Mitigation
                </h4>
                <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed font-medium">
                  {selectedAlert.type === "Idle" &&
                    "This resource has average CPU utilization below 5% for the past 7 days, indicating it is currently not being utilized. To optimize costs, terminate the instance or configure standard scheduling to shut it down during off-hours."}
                  {selectedAlert.type === "Oversized" &&
                    "The instance's maximum CPU and memory utilization are low, indicating it is oversized for its workload. Downsizing the instance to a smaller family (e.g. t3.medium to t3.small) will safely save up to 40% of compute cost."}
                  {selectedAlert.type === "UnattachedStorage" &&
                    "This S3 bucket has zero read/write operations over the past 7 days, indicating it is likely abandoned. Archive data to Glacier or delete the bucket completely to eliminate storage overhead."}
                </p>
              </div>
              ...
              <div className="mt-auto pt-4 flex flex-col sm:flex-row justify-center gap-3 border-t border-gray-100 dark:border-white/10">
                ...
                {selectedAlert.status === "resolved" && (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() =>
                      updateAlertStatus(selectedAlert._id, "archived")
                    }
          ...
          <div className="flex flex-col lg:flex-row justify-between items-center gap-4 mb-4">
            <div className="flex gap-1.5 bg-gray-100/50 dark:bg-white/5 p-1 rounded-xl w-fit flex-nowrap whitespace-nowrap overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {tabs.map((tab) => {
                ...
                const hasUnresolvedAlerts = alerts.some(
                ...
                return (
                  <motion.button
                    ...
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className={`relative px-2.5 sm:px-3 py-1 text-[11px] font-bold rounded-lg transition-all z-10 ${
                      activeTab === tab
                        ? "text-[#111844] dark:text-white"
                  ...
                  >
                    ...
                    <span
                      className={`relative z-10 ${shouldBlink ? "animate-pulse text-red-500 font-black drop-shadow-sm" : ""}`}
                    >
                      {tab}
                    </span>
                  </motion.button>
            ...
            <div className="flex items-center gap-2.5">
              <div className="relative" ref={envRef}>
                ...
                {isEnvOpen && (
                  <div className="absolute right-0 mt-1 w-32 bg-[#ffffff] dark:bg-[#0F122B] rounded-lg shadow-xl border border-gray-100 dark:border-slate-700 py-1 overflow-hidden z-30">
                    {uniqueEnvs.map((env) => (
                      <button
                        key={env}
                        onClick={() => {
            ...
            <AnimatePresence mode="popLayout">
              {isLoading ? (
                [0, 1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="bg-white border border-gray-100 p-3 rounded-xl flex items-center justify-between animate-pulse"
                  >
                  ...
                  {activeTab === "In progress" && (
                    <p className="font-medium text-gray-500">
                      ...
                      <button
                        ...
                        className="text-[#792CA2] hover:underline font-bold transition-all"
                      >
                        Unresolved
                      </button>{" "}
                      tab for new alerts to work on.
                    </p>
                  )}
                </div>
              ) : (
                sortedAlerts.map((alert, i) => (
                  <motion.div
                    ...
                    className={`bg-white dark:bg-[#111844]/50 border p-3.5 rounded-xl shadow-sm hover:shadow-md transition-colors duration-250 cursor-pointer flex flex-col group ${
                      selectedAlert?._id === alert._id
                        ? "border-2 border-[#792CA2] dark:border-[#9A4DCC] bg-[#792CA2]/5 dark:bg-[#9A4DCC]/10"
                        : "border-gray-100 dark:border-white/10 hover:border-[#792CA2]/30 dark:hover:border-white/30"
                    }`}
                  >
                    <div className="flex flex-col gap-1.5 w-full">
                      <div className="flex items-start justify-between w-full">
                        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 group-hover:text-[#792CA2] dark:group-hover:text-white transition-colors line-clamp-1">
                          {alert.message}
                        </span>
                        <ChevronDownIcon
                          className={`w-4 h-4 text-gray-300 dark:text-gray-500 transition-transform flex-shrink-0 mt-0.5 ${
                            selectedAlert?._id === alert._id
                              ? "rotate-0 text-[#792CA2] dark:text-[#9A4DCC]"
                        ...
                        >
                          <div className="flex justify-center pb-2">
                            <div className="flex items-center justify-between bg-gray-50/80 dark:bg-white/5 p-1.5 rounded-xl border border-gray-100 dark:border-white/10 shadow-sm w-full">
                              <button
                                onClick={() =>
                                  updateAlertStatus(alert._id, "unresolved")
                                }
                                className={`flex items-center justify-center flex-1 py-1.5 rounded-lg transition-all ${
                                  alert.status === "unresolved"
                                    ? "bg-red-100 dark:bg-red-500/20 border border-red-200 dark:border-red-500/30 shadow-sm"
                                    : "opacity-60 grayscale hover:opacity-100 hover:grayscale-0 hover:bg-red-50 dark:hover:bg-white/5"
                                }`}
                                title="Mark Unresolved"
                              >
                                <FlagIcon
                                  className={`w-4 h-4 ${alert.status === "unresolved" ? "text-red-500" : "text-gray-400"}`}
                                />
                              </button>

                              <button
                                onClick={() =>
                                  updateAlertStatus(alert._id, "in progress")
                                }
                                className={`flex items-center justify-center flex-1 mx-1.5 py-1.5 rounded-lg transition-all ${
                                  alert.status === "in progress"
                                    ? "bg-yellow-100 dark:bg-yellow-500/20 border border-yellow-200 dark:border-yellow-500/30 shadow-sm"
                                    : "opacity-60 grayscale hover:opacity-100 hover:grayscale-0 hover:bg-yellow-50 dark:hover:bg-white/5"
                                }`}
                                title="Start Progress"
                              >
                                <FlagIcon
                                  className={`w-4 h-4 ${alert.status === "in progress" ? "text-yellow-500" : "text-gray-400"}`}
                                />
                              </button>
                              ...
                              <button
                                ...
                                className={`flex items-center justify-center flex-1 py-1.5 rounded-lg transition-all ${
                                  alert.status === "resolved"
                                    ? "bg-green-100 dark:bg-green-500/20 border border-green-200 dark:border-green-500/30 shadow-sm"
                                    : "opacity-60 grayscale hover:opacity-100 hover:grayscale-0 hover:bg-green-50 dark:hover:bg-white/5"
                                }`}
                                title="Mark Resolved"
                              >
                                <FlagIcon
                                  className={`w-4 h-4 ${alert.status === "resolved" ? "text-green-500" : "text-gray-400"}`}
                                />
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="bg-white dark:bg-transparent border border-gray-150 dark:border-white/10 p-2.5 rounded-xl shadow-sm">
                              <p className="text-[8px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wide">
                                Current Cost
                              </p>
                              <p className="text-xs font-extrabold text-gray-600 dark:text-gray-300 mt-0.5">
                                ${Math.round(alert.currentCost || 0)}/mo
                              </p>
                            </div>
                            <div className="bg-emerald-500/5 border border-emerald-500/15 dark:border-emerald-500/30 p-2.5 rounded-xl shadow-sm">
                              <p className="text-[8px] font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-wide">
                                Potential Savings
                              </p>
                              <p className="text-xs font-black text-emerald-600 dark:text-emerald-500 mt-0.5">
                                ${Math.round(alert.potentialSavings || 0)}/mo
                              </p>
                            </div>
                          </div>

                          <div className="bg-white dark:bg-transparent border border-gray-150 dark:border-white/10 rounded-xl p-3 shadow-sm">
                            <h4 className="text-[9px] font-black text-[#111844] dark:text-gray-200 uppercase tracking-wider mb-1">
                              Recommended Mitigation
                            </h4>
                          ...
                          <div className="flex flex-col gap-2 mt-2">
                            ...
                            {alert.status === "resolved" && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  updateAlertStatus(alert._id, "archived");
                                }}
